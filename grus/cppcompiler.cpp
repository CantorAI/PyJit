...
bool CppCompiler::BuildCode(int moduleIndex,std::string strJitFolder,
	std::vector<std::string>& srcs, std::vector<std::string>& exports)
{
	const int online_len = 1024*8;
	const char* funcLineTemp =
		"\tpHost->AddFunc(idx,context,\"%s\",\"%s\", (void*)%s_stub);\n";
	const char* classLineTemp =
		"\tstatic const char* P[]={%s};\n"
		"\tstatic const unsigned long long M[]={\n\t\t%s\t\t};\n"
		"\tpHost->AddClass(idx,context,\n\t\t\t\"%s\",\"%s\",%d,%d,"
		"\n\t\t\tP,sizeof(P)/sizeof(const char*),"
		"\n\t\t\tM,sizeof(M)/sizeof(unsigned long long));\n";
	...
	std::string stubFileHead =
		"#include \"Jit_Object.h\"\n\n";
	stubFileHead += otherIncludes;

	const char* init_func_head_temp =
		"void %s(JitHost* pHost,void* context)\n"
		"{\n"
		"\tint idx = pHost->AddModule(context, \"%s\");\n";
	static std::string init_func_post =
		"}\n";
